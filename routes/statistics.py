...
def statistics():
    ...
    try:
        ...
        SELECT 
            ...
            a.art_title, 
            a.artist, 
            AVG(CAST(ur.rank AS FLOAT)) AS average_rank,
        ...
        JOIN user_rankings ur ON a.art_id = ur.art_id
            GROUP BY a.art_id, a.art_title, a.artist
        """)
        ...
        average_data = [int(round(artwork['average_rank'])) for artwork in average_sorted_artworks]
    
        # Filter artworks that have at least one top-3 ranking
        top_ranked_artworks = [
